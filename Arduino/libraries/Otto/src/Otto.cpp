...
//-- INITIALIZATION ---------------------------------------------//
///////////////////////////////////////////////////////////////////
void Otto::init(int YL, int YR, int RL, int RR, int Buzzer, int LH, int RH, bool load_calibration) {
...
//-- OSCILLATORS TRIMS ------------------------------------------//
///////////////////////////////////////////////////////////////////
void Otto::setTrims(int YL, int YR, int RL, int RR, int RH, int LH) {
  servo[0].SetTrim(YL);
  servo[1].SetTrim(YR);
  servo[2].SetTrim(RL);
  servo[3].SetTrim(RR);
...
void Otto::_moveServos(int time, int  servo_target[]) {
  ...
  if(time>10){
    for (int i = 0; i < 6; i++) increment[i] = (servo_target[i] - servo[i].getPosition()) / (time / 10.0);

    for (int iteration = 1; millis() < final_time; iteration++) {
      partial_time = millis() + 10;
      for (int i = 0; i < 6; i++) servo[i].SetPosition(servo[i].getPosition() + increment[i]);
...
void Otto::setArms(int LA, int RA) {
  // leftArmPin = LA;
  // rightArmPin = RA;
...
void Otto::moveArms(int leftAngle, int rightAngle) {
   if (servo_pins[4] != -1) {   // assuming 4 = left hand
        servo[4].SetPosition(leftAngle);
    }
    if (servo_pins[5] != -1) {   // assuming 5 = right hand
        servo[5].SetPosition(rightAngle);
    }
}
...
void Otto::_moveSingle(int position, int servo_number) {
  ...
  if (servo_number >=0 && servo_number < 6){
    servo[servo_number].SetPosition(position);
  }
}

void Otto::oscillateServos(int A[6], int O[6], int T, double phase_diff[6], float cycle) {
    for (int i=0; i<6; i++) {
        servo[i].SetO(O[i]);
        servo[i].SetA(A[i]);
        servo[i].SetT(T);
...
void Otto::_execute(int A[6], int O[6], int T, double phase_diff[6], float steps){
  attachServos();
  if(getRestState()==true){
        setRestState(false);
  }
  ...
  if (cycles >= 1)
    ...
    oscillateServos(A,O, T, phase_diff, 1.0);
...
void Otto::jump(float steps, int T){
  int up[]={90,90,150,30,90,90};
  _moveServos(T,up);
  int down[]={90,90,90,90,90,90};
  _moveServos(T,down);
}

void Otto::walk(float steps, int T, int dir){
  int A[6]= {30, 30, 20, 20, 0, 0};
  int O[6] = {0, 0, 4, -4, 0, 0};
  double phase_diff[6] = {0, 0, DEG2RAD(dir * -90), DEG2RAD(dir * -90), 0, 0};
  _execute(A, O, T, phase_diff, steps);
}

void Otto::walkhand(float steps,int T,int dir){
  int A[6]= {30, 30, 20, 20, 25, 25};
  int O[6] = {0, 0, 4, -4, 0, 0};
  double phase_diff[6] = {0, 0, DEG2RAD(dir * -90), DEG2RAD(dir * -90), DEG2RAD(dir * -90), DEG2RAD(dir * -90)};
  _execute(A, O, T, phase_diff, steps);
}
